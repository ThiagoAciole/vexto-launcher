# Makefile for Labs Launcher
# Precise setup for XFCE Panel detection

NAME = labs-launcher
SONAME = liblabs-launcher.so
...
ASSETDIR = $(PREFIX)/share/labs-launcher/icons
STYLEDIR = $(PREFIX)/share/labs-launcher/styles

all: $(SONAME) data/labs-launcher.desktop
...
data/labs-launcher.desktop: data/labs-launcher.desktop.in
	cp data/labs-launcher.desktop.in data/labs-launcher.desktop

clean:
	rm -f $(OBJ) $(SONAME) data/labs-launcher.desktop
...
install: all
	...
	install -m 644 data/labs-launcher.desktop $(DESTDIR)$(DATADIR)/
	install -m 644 data/icons/labs-launcher.svg $(DESTDIR)$(ICONDIR)/
...
uninstall:
	...
	rm -f $(DESTDIR)$(DATADIR)/labs-launcher.desktop
	rm -f $(DESTDIR)$(ICONDIR)/labs-launcher.svg
