...
static void ol_set_image_from_name_or_path(GtkImage *image,
    ...
    for (int i = 0; paths_to_check[i] != NULL; i++) {
      if (g_file_test(paths_to_check[i], G_FILE_TEST_EXISTS)) {
        GdkPixbuf *pixbuf = gdk_pixbuf_new_from_file_at_scale(
            paths_to_check[i], size, size, TRUE, NULL);
        if (pixbuf) {
  ...
  if (!found) {
    gtk_image_set_from_icon_name(image, "xfce4-panel-menu",
...
static void ol_update_icon(VextoLauncher *ol) {
  ...
  ol_set_image_from_name_or_path(GTK_IMAGE(ol->icon), ol->icon_name,
                                 target_size);
}
...
static void ol_icon_chooser_clicked(GtkWidget *button, VextoLauncher *ol) {
  ...
  GtkWidget *dialog = gtk_file_chooser_dialog_new(
      ...
      GTK_WINDOW(gtk_widget_get_toplevel(button)), GTK_FILE_CHOOSER_ACTION_OPEN,
      "Cancelar", GTK_RESPONSE_CANCEL, "Selecionar", GTK_RESPONSE_ACCEPT, NULL);
...
static void ol_configure(XfcePanelPlugin *plugin, VextoLauncher *ol) {
  ...
  g_signal_connect(icon_button, "clicked", G_CALLBACK(ol_icon_chooser_clicked),
                   ol);
