...
static void on_action_clicked(GtkWidget *btn, gpointer data) {
  ...
  if (g_strcmp0(cmd, "focus-search") == 0)
    return;
...
static gboolean on_search_key_press(GtkWidget *widget, GdkEventKey *event,
  ...
  if (event->keyval == GDK_KEY_Down) {
    /* Move focus to the first child of the grid */
    if (ol->grid) {
  ...
  return FALSE; /* Let other handlers run (e.g., typing text) */
}
...
void vexto_launcher_window_init(VextoLauncher *ol) {
  ...
  /* Folder Header (< [Categoria]) */
  ol->folder_header = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 8);
  gtk_widget_set_name(ol->folder_header, "vexto-launcher-folder-header");
  ...
  /* Fake VextoGridItem for the back button so on_item_clicked can reuse logic
   */
  VextoGridItem *back_item = vexto_grid_item_new_back();
  g_object_set_data(G_OBJECT(back_btn), "ol", ol);
  g_signal_connect(back_btn, "clicked", G_CALLBACK(on_item_clicked), back_item);
  /* Note: this leaks back_item, but we only create it once per window so it's
   * acceptable for now */
  ...
  /* Prev Button (Chevron) */
  GtkWidget *btn_prev =
      gtk_button_new_from_icon_name("pan-start-symbolic", GTK_ICON_SIZE_BUTTON);
  gtk_style_context_add_class(gtk_widget_get_style_context(btn_prev),
                              "nav-btn");
  ...
  /* Next Button (Chevron) */
  GtkWidget *btn_next =
      gtk_button_new_from_icon_name("pan-end-symbolic", GTK_ICON_SIZE_BUTTON);
  gtk_style_context_add_class(gtk_widget_get_style_context(btn_next),
                              "nav-btn");
  ...
  for (int i = 0; i < 4; i++) {
    GtkWidget *btn = gtk_button_new_from_icon_name(actions_data[i].icon,
                                                   GTK_ICON_SIZE_BUTTON);
    gtk_style_context_add_class(gtk_widget_get_style_context(btn),
                                "action-btn");
    if (g_strcmp0(actions_data[i].cmd, "focus-search") == 0) {
      g_signal_connect_swapped(
          btn, "clicked", G_CALLBACK(gtk_widget_grab_focus), ol->search_entry);
    } else {
      g_signal_connect(btn, "clicked", G_CALLBACK(on_action_clicked),
                       (gpointer)actions_data[i].cmd);
    }
...
void vexto_launcher_show(VextoLauncher *ol) {
  ...
  /* Show first, then update grid to properly hide/show inner components */
...
void vexto_launcher_hide(VextoLauncher *ol) {
  ...
  /* Reset folder before clearing search to ensure grid_update uses root */
